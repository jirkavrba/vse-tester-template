...
const Question: FC<QuestionProps> = ({text, answers, revealed, onSelect}: QuestionProps) => {
    return (
        <div className="mx-auto">
            <h1 className="text-3xl font-black text-center max-w-4xl mx-auto">{text}</h1>
            <div className="w-screen flex flex-col items-center">
                <ul className="flex flex-col items-stretch mt-10 w-1/2">
                    {answers.map((answer, i) => {
                        if (revealed) {
                            return <li key={i} className={`px-10 py-8 m-2 rounded-xl font-black ${answer.correct ? 'text-green-800 bg-green-200' : 'text-red-800 bg-red-200'}`}>{answer.text}</li>
                        }
